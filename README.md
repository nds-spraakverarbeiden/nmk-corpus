# Corpus of North Markian and Central Pomeranian (NMK Corpus)

> Note: This is the public release repository for the NMK corpus, for internal development, we use [a private repository](https://github.com/acoli-repo/nmk-corpus). If you like to contribute, please get in touch with [@chiarcos](https://github.com/chiarcos) and/or the [Chair of Applied Computational Linguistics at the University of Augsburg, Germany](https://www.uni-augsburg.de/en/fakultaet/philhist/professuren/Applied-Computational-Linguistics/).
